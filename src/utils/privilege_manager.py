...
class PrivilegeManager:
    ...
    async def request_privileges(self, operation: str) -> bool:
        ...
        if self.has_root:
            log_debug_safe(
                logger,
                safe_format("Already have root privileges for {op}", op=operation),
                prefix="PRIV",
            )
            return True
        ...
        if self.can_sudo:
            log_debug_safe(
                logger,
                safe_format("Sudo available without password for {op}", op=operation),
                prefix="PRIV",
            )
            return True
        ...
        if self.sudo_needs_password:
            log_warning_safe(
                logger,
                safe_format(
                    "Sudo is available for {op} but requires a password; "
        ...
        log_warning_safe(
            logger,
            safe_format("Cannot obtain privileges for {op}", op=operation),
            prefix="PRIV",
        )
        return False
    ...
    async def _request_sudo_permission(self, operation: str) -> bool:
        ...
        log_info_safe(
            logger,
            safe_format("Requesting permission to {desc} using sudo", desc=description),
            prefix="PRIV",
        )
    ...
    async def run_with_privileges(
        ...
        try:
            ...
            log_error_safe(
                logger,
                safe_format("Error running privileged command: {err}", err=str(e)),
                prefix="PRIV",
            )
            return False, "", str(e)
...
class PrivilegeRequest:
    ...
    async def request_dialog(app, operation: str, description: str) -> bool:
        ...
        log_info_safe(
            logger,
            safe_format(
                "Privilege elevation requested for {op}: {desc}",
                op=operation,
                desc=description,
            ),
            prefix="PRIV",
