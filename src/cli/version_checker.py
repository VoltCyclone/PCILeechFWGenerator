...
from ..string_utils import log_warning_safe, safe_format, log_debug_safe
...
def save_cache(latest_version: str, update_available: bool):
    ...
    try:
        ...
        log_debug_safe(
            logger, 
            safe_format("Failed to save cache, error={e}", e=str(e)),
            prefix="VERS"
        )
        pass
        ...
        if isinstance(e, HTTPError):
            try:
                ...
                log_debug_safe(
                    logger, 
                    safe_format(
                        "Failed to read GitHub response, error={e}", e=str(e)
                    ),
                    prefix="VERS"
                )
        else:
            log_debug_safe(
                logger,
                safe_format("GitHub fetch error: {e}", e=str(e)),
                prefix="VERS"
            )
        return None
...
def fetch_latest_version_pypi() -> Optional[str]:
    ...
    try:
        ...
        log_debug_safe(
            logger,
            safe_format("PyPI fetch error: {e}", e=str(e)),
            prefix="VERS"
        )
        return None
...
def fetch_latest_version() -> Optional[str]:
    ...
    try:
        ...
        log_debug_safe(
            logger,
            safe_format("Failed to fetch latest version: {e}", e=str(e)),
            prefix="VERS"
        )
        return None
...
def check_for_updates(force: bool = False) -> Optional[Tuple[str, bool]]:
    ...
    try:
        ...
        log_debug_safe(
            logger,
            safe_format("Error checking for updates: {e}", e=str(e)),
            prefix="VERS"
        )
        return None
...
def prompt_for_update(latest_version: str):
    ...
    log_warning_safe(
        ...
        "   If this is a pre-release version pull from git\n"
        "\n"
...
def check_and_notify():
    ...
    try:
        ...
        log_debug_safe(
            logger,
            safe_format("Version check failed: {e}", e=str(e)),
            prefix="VERS"
        )
        pass
